...
public class TotemOfFreezingCatalystAbility extends PassiveCatalystAbility {

    @Override
    public void postDamageEvent(LivingDamageEvent event) {
        if (ModChecker.friendsandfoes) {
            LivingEntity entity = event.getEntity();
            TotemEffectPacket.sendToClient(((Player) entity), new ItemStack(FriendsAndFoesItems.TOTEM_OF_FREEZING.get()));
            if (MobUtil.healthPercent(entity, 0.5F) && entity instanceof Player player) TotemUtil.freezeEntities(player);
