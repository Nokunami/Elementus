...
public class GolemAttackGoal extends MeleeAttackGoal {
    ...
    public GolemAttackGoal(TamableGolem golem, double pSpeedModifier, boolean pFollowingTargetEvenIfNotSeen) {
        super(golem, pSpeedModifier, pFollowingTargetEvenIfNotSeen);
        this.golem = golem;
        attackType = golem.getAttackType();
    ...
    protected void checkAndPerformAttack(@NotNull LivingEntity enemy, double distToEnemySqr) {
        if (distanceToStartAttackWindUp(enemy, distToEnemySqr)) {
            ...
            golem.setAttackPerformed(false);
            golem.setAttackType(0);
        }
    }
    ...
    protected void resetAttackCooldown() {
        ticksTilNextAttack = adjustedTickDelay(attackDelay * 2);
    }
    protected boolean isTimeToStartAttackAnimation() {
        if (golem.getAoeTimer() <= 0) {
            return false;
        } else {
            return ticksTilNextAttack <= attackDelay;
        }
    }
    protected boolean isTimeToAttack() {
        return ticksTilNextAttack <= 0;
    }
    protected int getTicksUntilNextAttack() {
        return ticksTilNextAttack;
    }
    ...
    public void tick() {
        ...
        if (!golem.isChassisCompromised()) {
            if (golem.getAoeTimer() > 20) {
                if (shouldCountTillNextAttack) ticksTilNextAttack = Math.max(ticksTilNextAttack - 1, 0);
    ...
    private void groundAttack(LivingEntity livingEntity) {
        if (golem.onGround()) {
            golem.playSound(SoundEvents.ANVIL_LAND, 1.4F, 1.2F);
            golem.playSound(SoundEvents.NETHERITE_BLOCK_BREAK, 1.4F, 0.8F);
