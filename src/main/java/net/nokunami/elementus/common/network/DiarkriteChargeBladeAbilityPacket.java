...
public class DiarkriteChargeBladeAbilityPacket {
    protected InteractionHand hand;

    public DiarkriteChargeBladeAbilityPacket(InteractionHand handIn) {
        hand = handIn;
    }

    public static void encode(DiarkriteChargeBladeAbilityPacket packet, FriendlyByteBuf buf) {
        buf.writeEnum(packet.hand);
    }

    public static DiarkriteChargeBladeAbilityPacket decode(FriendlyByteBuf buf) {
        return new DiarkriteChargeBladeAbilityPacket(buf.readEnum(InteractionHand.class));
    }

    public static void consume(final DiarkriteChargeBladeAbilityPacket packet, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
