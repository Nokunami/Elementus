...
import static net.nokunami.elementus.common.item.unique.BladeOfResonance.getCharge;
import static net.nokunami.elementus.common.item.unique.BladeOfResonance.getMaxCharge;
...
public class EItemPredicate {

     public static void registerItemPredicate() {
         shieldBlocking(EItems.ASTALITE_SHIELD.get());
    ...
    private static void catalystArmor(Item item) {
         ...
         ItemProperties.register(item, new ResourceLocation("elementus", "show_trim"), (itemStack, level, entity, i) ->
                 getTrimVisibility(itemStack) > 1 ? 0 : 1);
    }
...
//                     -> entity != null ? entity.getUseItem() != itemStack ? 0.0F : (float) (itemStack.getUseDuration() - entity.getUseItemRemainingTicks()) / 20.0F : 0.0F);
             ...
             ItemProperties.register(item, new ResourceLocation("pull"), (itemStack, level, entity, i) -> {
                 if (entity != null && entity.getUseItem() == itemStack && itemStack.getItem() instanceof EBowItem eb) {
                     return (float) (itemStack.getUseDuration() - entity.getUseItemRemainingTicks()) / eb.getPullLimit();
                 }
                 return 0;
             });
         }
